...
<!-- Suspected root cause — fill in manually -->"
  ...
  if [ -z "$REPORT_REPO" ]; then
    ...
    log_line "$(printf '[%s] status=fail event=%s sid=%s reason=repo_resolution_failed source=%s hook_cwd=%s local=%s' \
      "$TS" "$EVENT" "$SESSION" "${REPORT_REPO_SOURCE:-unknown}" "${HOOK_CWD:-}" "$LOCAL_OK")"
